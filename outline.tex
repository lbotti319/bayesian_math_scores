...
$$\texttt{Absences}_i|\texttt{Age}_i \sim \texttt{Poisson}(\mu_i)$$
...
Our parameters are then $\alpha_0, \alpha_1, \gamma_0, \gamma_1, \texttt{Absences}_i, \texttt{Higher Yes}_i, \mathbf{\beta},$ and $\sigma^2$.

The full posterior is:

$$p(\alpha_0, \alpha_1, \gamma_0, \gamma_1, \texttt{Absences}_i, \texttt{Higher Yes}_i, \mathbf{\beta},\sigma^2|\mathbf{G3}, \mathbf{X}_{\texttt{Rest}})$$

$$\propto L(\mathbf{G3}|\alpha_0, \alpha_1, \gamma_0, \gamma_1, \texttt{Absences}_i, \texttt{Higher Yes}_i, \mathbf{\beta},\sigma^2, \mathbf{X}_{\texttt{Rest}})$$
$$\prod_{i=1}^{m}\pi(\texttt{Absences}_i|\texttt{Age}_i)\prod_{i=1}^{n}\pi(\texttt{Higher Yes}_i|\texttt{Age}_i)\pi(\beta, \sigma^2)\pi(\alpha_0)\pi(\alpha_1)\pi(\gamma_0)\pi(\gamma_1)$$


We assume the following priors

$$\pi(\beta, \sigma^2)\propto (\sigma^2)^{-1}$$ and $\alpha_0, \alpha_1, \gamma_0, \gamma_1$ are given $N(0, 100)$.
...
$$\prod_{i=1}^m\frac{(\exp[\gamma_o+\gamma_1\texttt{Age}_i])^{\texttt{Absences}_i}\exp[-\exp[\gamma_o+\gamma_1\texttt{Age}_i]]}{\texttt{Absences}_i}!}$$
...
$$\exp[-\frac{\alpha_0^2}{200}-\frac{\alpha_1^2}{200}-\frac{\gamma_0^2}{200}-\frac{\gamma_1^2}{200}]$$
...
$$p(\texttt{Absences}_i|\texttt{everything else}) \sim \texttt{Poisson}(\exp[\gamma_0+\gamma_1\texttt{Age}_i])$$
...
$$\propto \prod_{i=1}^{n}(1-\frac{\exp[\alpha_0+\alpha_1\texttt{Age}_i]}{1+\exp[\alpha_0+\alpha_1\texttt{Age}_i]})^{1-\texttt{Higher Yes}_i}(\frac{\exp[\alpha_0+\alpha_1\texttt{Age}_i]}{1+\exp[\alpha_0+\alpha_1\texttt{Age}_i]})^{\texttt{Higher Yes}_i}\exp[-\frac{\alpha_0^2}{200}]$$

$$p(\alpha_1|\texttt{everything else})$$

$$\propto \prod_{i=1}^{n}(1-\frac{\exp[\alpha_0+\alpha_1\texttt{Age}_i]}{1+\exp[\alpha_0+\alpha_1\texttt{Age}_i]})^{1-\texttt{Higher Yes}_i}(\frac{\exp[\alpha_0+\alpha_1\texttt{Age}_i]}{1+\exp[\alpha_0+\alpha_1\texttt{Age}_i]})^{\texttt{Higher Yes}_i}\exp[-\frac{\alpha_1^2}{200}]$$

$$p(\gamma_0|\texttt{everything else})$$

$$\propto \prod_{i=1}^m(\exp[\gamma_0+\gamma_1\texttt{Age}_i])^{\texttt{Absences}_i}\exp[-\exp[\gamma_o+\gamma_1\texttt{Age}_i]]}\exp[-\frac{\gamma_0^2}{200}]$$

$$p(\gamma_1|\texttt{everything else})$$

$$\propto \prod_{i=1}^m(\exp[\gamma_0+\gamma_1\texttt{Age}_i])^{\texttt{Absences}_i}\exp[-\exp[\gamma_o+\gamma_1\texttt{Age}_i]]}\exp[-\frac{\gamma_1^2}{200}]$$
