...
\subsubsection{Joint Posterior}

Let $\mathbf{\Sigma} = \sigma^2\mathbf{I}$. Thus the joint posterior is

$$p(\vec\beta, \sigma^2, X_{mis, G2}, X_{mis, Higher}) \propto \mathcal{L}(\vec{Y}|\mathbf{X}\vec\beta, \sigma^2)\mathcal{L}(X_{mis, GS}|\mathbf{X}_{mis, Age}\vec\gamma, \eta^2)\mathcal{L}(X_{i, Higher}|z_i, \theta^2)\pi(\vec\beta, \sigma^2)\pi(\gamma, \eta^2)\pi(z_i, \theta^2)$$


$$p(\vec\beta, \sigma^2, X_{mis, G2}, X_{mis, Higher}) \propto |\Sigma|^\frac{-1}{2} exp\left(\frac{-1}{2}(\vec{Y} - \mathbf{X}\vec\beta)^T\mathbf{\Sigma}^{-1}(\vec{Y} - \mathbf{X}\beta)\right)$$ 
$$|\eta^2\mathbf{I}|^\frac{-1}{2} exp\left(\frac{-1}{2}(X_{mis, GS} - \mathbf{X}_{mis, Age}\vec\gamma)^T\eta^2\mathbf{I}^{-1}(\vec{Y} - \mathbf{X}_{mis, Age}\vec\gamma)\right)$$
$$(\theta)^\frac{n}{2} exp\left[\frac{-1}{2\theta^2}(X_{i, Higher} - z_i)^2\right](\sigma^2)^{-1}(\eta^2)^{-1} \cdot 1$$
...
$X_{mis, G2} \sim \vec\gamma, \eta, X_{mis, Age} \sim$

2.

$X_{mis, Higher} \sim\alpha_0, \alpha_1, X_{mis, Age} \sim$

3.

$\vec\beta | \sigma^2, y, \mathbf{X} \sim$

$\sigma^2 | \vec\beta, y, \mathbf{X} \sim$

4.
$\vec{\gamma}|\eta^2, X_{obs, GS}, X_{obs, Age}, X_{mis, GS}, X_{mis, Age} \sim$

$\eta^2|\vec{\gamma}, X_{obs, GS}, X_{obs, Age}, X_{mis, GS}, X_{mis, Age} \sim$

5.

$\vec\alpha| \theta^2, X_{obs, Higher}, X_{obs, Age}, X_{mis, Higher}, X_{mis, Age} \sim$ 

$\theta^2|\vec\alpha, X_{obs, Higher}, X_{obs, Age}, X_{mis, Higher}, X_{mis, Age} \sim$ 


\subsubsection{Gibbs Sampler}

Use the sample mean for G2, "Yes" for Higher (sample mode), and OLS estimates from the complete data for everything else for all starting values. The Gibbs sample has the following steps:

\begin{enumerate}
  \item Impute $X_{mis, G2}$ given $\vec\gamma, \eta, X_{mis, Age}$
  \item Impute $X_{mis, Higher}$ given $\alpha_0, \alpha_1, X_{mis, Age}$
  \item Draw $(\vec\beta, \sigma^2)$ given $y, \mathbf{X}$
  \item Draw $(\vec{\gamma}, \eta)$ given $X_{obs, GS}, X_{obs, Age}, X_{mis, GS}, X_{mis, Age}$
  \item Draw $(\vec\alpha, \theta^2)$ given $X_{obs, Higher}, X_{obs, Age}, X_{mis, Higher}, X_{mis, Age}$  
\end{enumerate}
